...
in <- dbConnect(SQLite(), DB_PATH)
ingest_proquest(file.path(PATH_RAW, "FT_AugSep2005.txt"), in, boilerplate_patterns = "<[^>]+>")
ingest_proquest(file.path(PATH_RAW, "FT_Oct2005.txt"), in, boilerplate_patterns = "<[^>]+>")
update_token_index(in, remove_stop_words = FALSE)
...
vocab_stats <- vocab_stats %>%
  mutate(
    ...
    Type = case_when(
      Residual < 0 ~ "Bursty (High Info)",
      Residual > 0 ~ "Generic (Low Info)",
...
# Document term matrix: Full and reduced

```{r}
# --- LEXICON SELECTION ---
# Full Lexicon: The original 16,796 terms
# Signal Lexicon: The 5,397 terms with Entropy_Gap < 0

signal_terms <- vocab_stats %>% 
  filter(Entropy_Gap < 0) %>% 
  pull(Term)

# Create two DTM versions
dtm_full   <- dtm
dtm_signal <- dtm[, signal_terms]


```

# Full and reduced compared
...
p_zipf <- ggplot(zipf_data, aes(x = Rank, y = TF, color = Type)) +
    # Use lines to show the curve
    geom_line(size = 1.2, alpha = 0.8) +
    
    # Log-Log scales are essential for Zipf's Law
    scale_x_log10(labels = scales::comma) +
    scale_y_log10(labels = scales::comma) +
    
    scale_color_manual(values = c("Full Lexicon (16,796)" = "gray70", 
                                  "Reduced Lexicon (5,397)" = "blue")) +
    
    theme_minimal() +
    labs(
        title = "Zipf's Law: Full vs. Reduced Lexicon",
        subtitle = "Comparing the frequency distribution after removing 'Glue' (Positive Entropy Gap)",
        x = "Rank (Log10 Scale)",
        y = "Total Frequency (Log10 Scale)",
        color = "Lexicon"
    )

# Save the diagnostic
ggsave("diagnostic_zipf_comparison.png", p_zipf, width = 9, height = 6)
print(p_zipf)
```
