...
# Model comparison: Reduced lexicon
...
sweep_results <- perform_knn_sweep(
  corpus = corpus,
  method = "jaccard", # CHANGE THIS to match your winning method code (e.g. "tfidf", "bm25")
  k_levels = 1:20
)
...
my_colors <- c(
  ...
  "Modularity"                              = "yellow",   # High contrast
  "Structural stress (Average betweenness)" = "pink",   # Deep blue
  "Bridge strength (Maximum betweenness)"   = "cyan",     # Distinct from blue
  "Survivors (%)"                           = "green",  # Warm but dark
  "Cluster count"                           = "darkorange2"   # Visible orange instead of yellow
)

ggplot(sweep_results, aes(x = k)) +
  ...
  labs(
    ...
    x = "K (number of near nearest neighbors retained)",
    color = "Metric"
  ) +
...
# Validated network metrics in k-NN graphs of documents

## Full lexicon

```{r}
# 1. Run the Sweep
# k=1 to k=10 is sufficient to see the phase transition and hub collapse
sweep_results_full <- run_knn_sweep(corpus, method = "jaccard", k_levels = 1:10, n_rand = 20)

# 2. Plot Small World Properties (Clustering)
# This confirms "The Archipelago" (Observed should be much higher than Random)
plot_knn_metric(sweep_results_full, "Transitivity", "Clustering Proof: Transitivity vs Random")

# 3. Plot Global Reach (Path Length)
# This confirms "The Highways" (Observed should be close to Random)
plot_knn_metric(sweep_results_full, "PathLength", "Efficiency Proof: Path Length vs. Chance")

# 4. Plot Disassortativity
# This confirms "The Hubs" (Observed should be negative, Random should be near 0)
plot_knn_metric(sweep_results_full, "Assortativity", "Structure Proof: Disassortativity vs. Random")

# 4a. Table of results 

get_comparison_table(sweep_results_full)

# 5. Check the Sigma Score
# The peak value indicates the optimal k for your app
print(sweep_results_full %>% select(k, Sigma, GCR, Max_Betweenness))
```
...
# 1. Run the Sweep
# k=1 to k=10 is sufficient to see the phase transition and hub collapse
sweep_results_red <- run_knn_sweep(corpus_signal, method = "jaccard", k_levels = 1:10, n_rand = 20)

# 2. Plot Small World Properties (Clustering)
# This confirms "The Archipelago" (Observed should be much higher than Random)
plot_knn_metric(sweep_results_red, "Transitivity", "Clustering Proof: Transitivity vs Random")

# 3. Plot Global Reach (Path Length)
# This confirms "The Highways" (Observed should be close to Random)
plot_knn_metric(sweep_results_red, "PathLength", "Efficiency Proof: Path Length vs. Chance")

# 4. Plot Disassortativity
# This confirms "The Hubs" (Observed should be negative, Random should be near 0)
plot_knn_metric(sweep_results_red, "Assortativity", "Structure Proof: Disassortativity vs. Random")

# 4a. Table of results 

get_comparison_table(sweep_results_red)

# 5. Check the Sigma Score
# The peak value indicates the optimal k for your app
print(sweep_results_red %>% select(k, Sigma, GCR, Max_Betweenness))
```
...
```{r}
# Find the "Accelerator Terms" that appear at k=3
new_bridges <- compare_bridge_evolution(corpus, k_low = 2, k_high = 3, top_n = 200)

print("Full lexicon")
print(head(new_bridges, 376))

new_bridges_red <- compare_bridge_evolution(corpus_signal, k_low = 2, k_high = 3, top_n = 200)

print("Reduced lexicon")
print(head(new_bridges_red,376))
